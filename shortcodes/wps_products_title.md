...
Displays the product "title" component.<br><br>Watch our [quick video tutorial](https://www.youtube.com/watch?v=lYm6G35e8sI) to learn how to use this.
...
```js
[wps_products_title tag="Tag A, Tag B"]
```
...
```js
[wps_products_title vendor="Vendor A, Vendor B"]
```
...
```js
[wps_products_title product_type="Books"]
```
...
```js
[wps_products_title available_for_sale="available"]
```
...
```js
[wps_products_title created_at="2019-02-21 03:16:41"]
```
...
```js
[wps_products_title updated_at="2019-02-21 03:16:41"]
```
...
```js
[wps_products_title sort_by="price"]
```
...
[wps_products_title sort_by="title" reverse="true"]
```
...
[wps_products_title page_size="4"]
```
...
[wps_products_title page_size="2" limit="10"]
...
[wps_products_title title="Awesome product, Even better product" connective="or"]

// Only products with both Tag1 and Tag2 will show
[wps_products_title tag="Tag1, Tag2" connective="and"]
```
...
```js
[wps_products_title items_per_row="5"]
```
...
```js
[wps_products_title no_results_text="Nothing found! 🙃"]
```
...
```js
[wps_products_title infinite_scroll="true"]
```
...
```js
[wps_products_title infinite_scroll_offset="-100"]
```
...
```js
[wps_products_title dropzone_load_more="#my-custom-load-more-container"]
```
